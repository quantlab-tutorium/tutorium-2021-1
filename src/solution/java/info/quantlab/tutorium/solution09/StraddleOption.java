...
public class StraddleOption extends AbstractAssetMonteCarloProduct {
	...
	public RandomVariable getValue(final double evaluationTime,
		...
		RandomVariable valuesCall	= underlyingAtMaturity.sub(strike).floor(0.0);
		RandomVariable valuesPut	= underlyingAtMaturity.sub(strike).mult(-1).floor(0);

		RandomVariable values		= valuesCall.add(valuesPut);
